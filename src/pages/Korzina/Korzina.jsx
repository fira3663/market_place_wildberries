          ...
          {products.map((elem) => {
            return (
              <div key={elem.product.id}>
                <div className="flex w-[80%] m-auto justify-evenly">
                  <div>
                    <img
                      src={elem.product.media.map((el) => {
                        return el.src;
                      })}
                      alt=""
                    />
                    ...
                    <button
                      onClick={() =>
                        dispatch(decrementProduct(elem.product.id))
                      }
                  ...
                  <div>
                    <p className="text-violet-500">Выбрать адресс доставки</p>
                    <p className="mt-2">Товар {elem.quantity}</p>
                    <p className="text-2xl mt-2">Итого {total} Р</p>
                    <button className="p-5 bg-violet-500 w-[100%] rounded-xl">Заказать</button>
                  </div>
                </div>
              </div>
            );
